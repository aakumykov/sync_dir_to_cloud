...
interface SyncOperationLoggerDAO {
    ...
    @Query("SELECT * FROM ${SyncOperationLogItem.TABLE_NAME} WHERE id = :id")
    suspend fun get(id: String): SyncOperationLogItem?


    @Query("SELECT * FROM ${SyncOperationLogItem.TABLE_NAME} WHERE " +
            "task_id = :taskId " +
    ...
    @Query("UPDATE ${SyncOperationLogItem.TABLE_NAME} SET " +
